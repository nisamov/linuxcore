...
- **Complejidad de Escalabilidad de Claves:** Requiere una clave única por cada par de entidades que deseen establecer una comunicación confidencial. En una red de $n$ participantes, el número total de claves necesarias sigue un crecimiento cuadrático:
   $$\text{Total de claves} = \frac{n(n - 1)}{2}$$
...
- Asignación de Claves por Nivel de Confidencialidad:
  - Clave Pública ($K_{pub}$): Accesible de manera irrestricta en el dominio público o distribuida mediante infraestructura de clave pública (PKI). Se emplea para cifrar mensajes destinados al propietario de la clave o para verificar firmas digitales.
  - Clave Privada ($K_{priv}$): Secretismo absoluto y custodio exclusivo por parte del titular. Se utiliza para descifrar datos entrantes o generar firmas digitales autenticadas.
